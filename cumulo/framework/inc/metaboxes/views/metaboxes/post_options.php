<ul class="cmo_metabox_tabs">
	<li class="active"><a href="#post"><?php echo __( "Post", 'cumulo' ) ?></a></li>
...
<div class='cmo_metabox'>
	<div class="cmo_metabox_tab active" id="cmo_tab_post">
		...
		?>
		<?php
		$this->select ( 'video_source', __ ( 'Audio/Video Source', 'cumulo' ), array (
				'embed' => __ ( 'Embed Code', 'cumulo' ),
				'file' => __ ( 'File', 'cumulo' ) 
		), __ ( 'Select audio/video source for audio/video post format.', 'cumulo' ) );
		$this->upload ( 'video_file', 
			__( 'URL of File for Audio/Video', 'cumulo' ), 
			__( 'Select audio/video file from media library or copy and paste url of file from remote server. Only mp4, flv, m4v, ogv, webm, wmv for videos, mp3, ogg, wav, wma for audio are allowed.', 'cumulo' ) 
		);
		?>
		<?php
		$this->textarea ( 'video', __ ( 'Video / Audio Embed Code', 'cumulo' ), __ ( 'Copy and paste your favorite youtube, vimeo or soundcloud embed code.', 'cumulo' ) );
		?>
	</div>
