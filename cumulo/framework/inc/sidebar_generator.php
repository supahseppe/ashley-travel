...
// 		add_action('edit_page_form', array('sidebar_generator', 'edit_form'));
	    ...
	    if(is_array($sidebars)){
			foreach($sidebars as $sidebar){
				...
				register_sidebar(array(
					'id' => 'cmo-' . strtolower( $sidebar_class ),
					'name' => $sidebar,
			    	'before_widget' => '<div id="%1$s" class="widget cmo-widget clearfix %2$s">',
		   			'after_widget' => '</div>',
		   			'before_title' => '<h3 class="widgettitle sbg_title">',
					'after_title' => '</h3>',
		    	));
			}
		}
	...
	static function add_sidebar(){
		$sidebars = sidebar_generator::get_sidebars();
		$name = str_replace( ' ', '-', $_POST['sidebar_name'] );
	...
	static function remove_sidebar(){
		...
		$id = sidebar_generator::name_to_class($name);
		if(!isset($sidebars[$id])){
			die("alert('" . __( 'Sidebar does not exist.', 'cumulo' ) . "')");
		}
	...
	static function admin_menu(){
		add_theme_page(__( 'Sidebars', 'cumulo' ), 'Sidebars', 'manage_options', 'sidebar_generator', array('sidebar_generator','admin_page'));
	}
	...
	static function admin_page(){
		...
		<script>
			...
			function add_sidebar_link(){
				var sidebar_name = prompt("<?php echo esc_html__( 'Sidebar Name', 'cumulo' ) ?>:","");
				if ( sidebar_name != null )
					add_sidebar(sidebar_name);
			}
		</script>
		<div class="wrap">
			<h2><?php echo __( 'Sidebar Generator', 'cumulo' ) ?></h2>
			<p>
				<?php echo __( 'The sidebar name is for your use only. It will not be visible to any of your visitors.', 'cumulo' ) ?>
				<?php echo __( 'A CSS class is assigned to each of your sidebar, use this styling to customize the sidebars.', 'cumulo' ) ?>
			</p>
			<br />
			<div class="add_sidebar">
				<a href="javascript:void(0);" class='button' onclick="return add_sidebar_link()" title="Add a sidebar">+ <?php echo __( 'Add Sidebar', 'cumulo' ) ?></a>
			</div>
			<br />
			<table class="widefat page" id="sbg_table" style="width:600px;">
				<tr>
					<th><?php echo __( 'Name', 'cumulo' ) ?></th>
					<th><?php echo __( 'CSS class', 'cumulo' ) ?></th>
					<th><?php echo __( 'Remove', 'cumulo' ) ?></th>
				</tr>
				<?php
				$sidebars = sidebar_generator::get_sidebars();
				...
				<tr class="<?php echo esc_attr( $alt )?>">
					...
					<td><a href="javascript:void(0);" onclick="return remove_sidebar_link('<?php echo esc_attr( $sidebar ); ?>',<?php echo intval($cnt) + 1; ?>);" title="<?php echo __( 'Remove this sidebar', 'cumulo' ) ?>">remove</a></td>
				</tr>
				<?php
						$cnt++;
					}
					...
					?>
					<tr>
						<td colspan="3"><?php echo __( 'No Sidebars defined', 'cumulo' ) ?></td>
					</tr>
					<?php
				}
				?>
	...
	*/
	...
	?>
	<div class="cmo_metabox_field">
		<input name="sbg_edit" type="hidden" value="sbg_edit" />
		<div class="cmo_desc">
			<label><?php echo __( 'Select Sidebar', 'cumulo' ) ?>:</label>
			<p><?php echo __( 'Select sidebar. Choose "No Sidebar" for full width.', 'cumulo' ) ?></p>
		</div>
		<div class="cmo_field">
		<?php 
		global $wp_registered_sidebars; ?>
			<select name="sidebar_generator[0]" style='display: none !important;'>
				<option value="0"<?php if($selected_sidebar[0] == ''){ echo " selected";} ?>><?php echo __( 'Default', 'cumulo' )?></option>
				<option value="-1"<?php if($selected_sidebar[0] == '-1'){ echo " selected";} ?>><?php echo __( 'No Sidebar', 'cumulo' )?></option>
			<?php
			$sidebars = $wp_registered_sidebars;// sidebar_generator::get_sidebars();
			if(is_array($sidebars) && !empty($sidebars)){
				foreach($sidebars as $sidebar){
					if($selected_sidebar[0] == $sidebar['id']){
			...
			<select name="sidebar_generator_replacement[0]">
				<option value="0"<?php if($selected_sidebar_replacement[0] == ''){ echo " selected";} ?>><?php echo __( 'Default', 'cumulo' ) ?></option>
				<option value="-1"<?php if($selected_sidebar_replacement[0] == '-1'){ echo " selected";} ?>><?php echo __( 'No Sidebar', 'cumulo' )?></option>
			<?php
			...
			if(is_array($sidebar_replacements) && !empty($sidebar_replacements)){
				foreach($sidebar_replacements as $sidebar){
					if($selected_sidebar_replacement[0] == $sidebar['id']){
						echo "<option value='{$sidebar['id']}' selected>{$sidebar['name']}</option>\n";
					}else{
						echo "<option value='{$sidebar['id']}'>{$sidebar['name']}</option>\n";
					}
...
$cmo_sidebarg = new sidebar_generator;
